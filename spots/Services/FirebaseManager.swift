...
//    }
    ...
    func addRatingIDToUser(ratingID: String) {
        FireIntegration.shared.addPostIDToUser(postID: ratingID)
//        let uid = getCurrentUserID()
//        let userRef = fs.collection("users").document(uid)
//        userRef.updateData(["ratedPosts": FieldValue.arrayUnion([ratingID])])
    ...
    func addPostToRated(postID: String) {
        FireIntegration.shared.addPostToRated(postID: postID)
//        let uid = getCurrentUserID()
//        let userRef = fs.collection("users").document(uid)
//        userRef.updateData(["ratedPosts": FieldValue.arrayUnion([postID])])
    ...
    func uploadImage(uuidArray: [String], data: [Data]) async throws {
        try await FireIntegration.shared.uploadImage(uuidArray: uuidArray, data: data)
...
//        }
    ...
    func getImagesByUUID(uuids: [String]) async throws -> [UIImage] {
        try await FireIntegration.shared.getImagesByUUID(uuids: uuids)
        // boiler plate func
//        var images: [UIImage] = []
...
//        return images
    ...
    func getImageURLs(uuids: [String]) -> [URL] {
        return FireIntegration.shared.getImageURLs(uuids: uuids)
//        var urls: [URL] = []
//        for id in uuids {
...
//        return urls
    ...
    func getFileSize(ref: StorageReference) async throws -> Int64 {
        return try await FireIntegration.shared.getFileSize(ref: ref)
...
//        return metadata.size
    ...
    func downloadData(ref: StorageReference, size: Int64) async throws -> Data {
        return try await FireIntegration.shared.downloadData(ref: ref, size: size)
